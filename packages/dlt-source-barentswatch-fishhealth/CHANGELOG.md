...
- Initial release: a dlt source for the BarentsWatch Fish Health API — the list of salmonoid localities (`localities_with_salmonoids`) and the detailed weekly report per locality (`locality_week`): lice counts, lice treatments, diseases, escapes, control areas and the aquaculture-register entry behind the locality. What each resource loads and how to configure it is in the [README](README.md).
...
- `localities`: every aquaculture locality in the register, salmonoid or not, from `GET /v1/geodata/fishhealth/localities` — about 2 700 against the salmonoid list's 2 000, with `municipalityNo`, `municipality` and `aquaCultureRegistryVersion` besides number and name. Loaded with `replace` when selected, and not otherwise; `locality_week` still iterates over `localities_with_salmonoids`. The two lists are named after their endpoints so they can be told apart — [which is which](CONTEXT.md#places).
