...
Two public localities over three fixed weeks: eight requests, a few seconds. The assertion
is that rows landed in the shape the offline suite assumes.
"""
...
def test_two_localities_three_weeks_land():
    ...
    assert 0 < len(weeks) <= len(LOCALITY_NOS) * len(WEEK_RANGE), "Expected at least one reported week"
    for row in weeks:
