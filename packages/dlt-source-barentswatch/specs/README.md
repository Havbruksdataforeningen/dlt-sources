...
`fishhealth.json` is the BarentsWatch Fish Health API's own OpenAPI document, and the source
of truth for the `fishhealth` source. One document per API, named after it, as the package
takes on more of them. Everything about the API itself — auth, base URL, the terms of
...
fixtures against its schemas. The filename stays put, so `git log -p specs/fishhealth.json`
...
the API actually returns, not against what `fishhealth.json` says it should. The differences
...
The API does a few things `fishhealth.json` does not describe. The source does not paper over
