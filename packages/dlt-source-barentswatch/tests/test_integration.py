...
def require_credentials():
    ...
    try:
        client_id = dlt.secrets.get("sources.fishhealth.client_id", str)
    except KeyError:
        client_id = None
    if not client_id:
        pytest.fail(
            "No BarentsWatch client id found. Provide credentials via either:\n"
            "  - SOURCES__FISHHEALTH__CLIENT_ID and ..__CLIENT_SECRET environment variables\n"
...
def test_the_full_locality_list_lands():
    ...
    source = fishhealth_source().with_resources("localities")
