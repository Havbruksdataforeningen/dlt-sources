"""The packaged `.dlt/*.example` files are checked by using them: copied into an empty dlt project and resolved with no arguments and no environment variables.

The README inlines the same two files for a reader on PyPI, so it is held to them as well.
"""
...
def source_from_examples(isolated_run_context: Path) -> Any:
    """Build the source from the example files alone, following the quick start literally."""
    ...
    # dlt reads the toml files when the run context loads, so point it at them again now they exist.
...
def _commented_resource_params(config_example: str, prefix: str) -> dict[str, list[str]]:
    """Parse the commented-out `# [<prefix><resource>]` sections into resource -> params; a dotted one names a table-valued argument."""
