...
- **A test run writes only inside its own package directory and dlt's data dir**, and **cleans up after itself**. A run produces a `.duckdb` per pipeline and dlt's state under `~/.dlt/pipelines/`, which is what `dlt pipeline <name> show` reads; both are deleted when the session finishes, because artifacts that accumulate silently are the ones that surprise someone later. Those artifacts are still the easiest way to see what a test ingested, so **give your package a flag that keeps them**: `dlt-source-aquabyte` takes `pytest --keep-db`. Teardown only removes what the run touched, so a kept artifact survives a later run of other tests. Nothing is ever written outside those two places, and nothing reaches git.
