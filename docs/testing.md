...
- **A test run writes only inside its own package directory and dlt's data dir**, and everything it writes is gitignored. What a run leaves behind is a feature — a `.duckdb` per pipeline you can open, and dlt's state under `~/.dlt/pipelines/`, which is what `dlt pipeline <name> show` reads — so inspecting what a test actually ingested needs no special run. It does accumulate, so **give your package a way to clear it**: `dlt-source-aquabyte` takes `pytest --clean-db`, which at the end of the session deletes the artifacts that run touched and leaves the rest. Nothing is ever written outside those two places, and nothing reaches git.
